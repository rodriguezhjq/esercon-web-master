<template>
  <div>
    ...
    <div class="flex items-center mt-8">
      <h2 class="intro-y text-lg font-medium mr-auto">Editar {{ nombre }}</h2>
    </div>
    <!-- BEGIN: Wizard Layout -->
    <div
      v-show="loading"
    ...
    <div class="intro-y box py-10 sm:py-20 mt-5" v-show="!loading">
      <div
      ...
      >
        ...
        <div id="faq-accordion-2" class="accordion accordion-boxed mt-20">
                      ...
                      <table class="table">
                        ...
                        <tbody>
                          <tr
                            ...
                            v-for="(item, i) in recorridos.filter(
                              item => item.estado != 'delete'
                            )"
                            :key="i"
                          >
          ...
          <EconomicCodition
            ...
            :key="item.id"
            :name="item.name"
            :conditionid="item.nombre_condicion_economica_id"
            :objeto="item"
      ...
      >
        ...
        <div class="intro-y grid grid-cols-12 gap-3 sm:gap-6 mt-5">
          <div class="intro-y col-span-3">
            <div
              class="file box rounded-md px-5 pt-2 pb-2 px-3 sm:px-5 zoom-in"
            >
              ...
              <a class="block font-medium mt-4 text-center truncate">
                {{ file.id ? file.nombre : 'Documento no adjunto' }}
              </a>
              <div
                v-if="file.id"
              ...
              >
                ...
                <div class="dropdown-menu w-40">
                  <div class="dropdown-menu__content box dark:bg-dark-1 p-2">
                    ...
                    >
                      <UsersIcon class="w-4 h-4 mr-2" /> Descargar
                    </a>
                    <a
                      href=""
                      class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2  rounded-md"
          ...
          <div class="intro-y col-span-9">
            <div id="single-file-upload" class="p-1">
              <div class="preview">
                <Dropzone
      ...
      >
        ...
        <button class="btn btn-primary w-24 ml-2" v-if="loading">
          ...
          <div class="modal-body p-0">
            <div class="p-5 text-center">
              ...
              <div class="text-gray-600 mt-2">
                ...
                Esto se eliminará temporalmente, al momento de presionar el
                boton "actualizar" este proceso no se podra deshacer.
              </div>
            </div>
            <div class="px-5 pb-8 text-center">
              <button
    ...
    <div
      id="delete-modal-condicion-preview"
      class="modal"
      tabindex="-1"
      aria-hidden="true"
    >
...
export default {
  ...
  data() {
    return {
      ...
      tipooperacionrellenoOLD: false,
      tipooperacionexcavacion: false,
      tipooperacionexcavacionOLD: false,
      tipooperacioninterno: false,
      tipooperacioninternoOLD: false,
      tipooperacionotro: false,
      tipooperacionotroOLD: false,
      disabledTipooperacion: true,
      otrotipooperacion: '',
      tipoviapavimentada: false,
      tipoviapavimentadaOLD: false,
      tipoviadestapada: false,
      tipoviadestapadaOLD: false,
      tipoviatrocha: false,
      tipoviatrochaOLD: false,
      tipoviaotro: false,
      tipoviaotroOLD: false,
      disabledtipovia: true,
      otrotipodevia: '',
      dobletroque: 'propio',
      cantidadasignadopropio: '',
      valorm3propio: '',
  ...
  methods: {
    borrarRecorrido(recorrido) {
      this.recorridos = this.recorridos.map(function(item) {
        if (item.id == recorrido.id) {
          item.estado = 'delete'
        }
        return item
      })
    ...
    eliminarPago(pago) {
      this.deleteCondicionEconomicaTemp = pago
      cash('#delete-modal-condicion-preview').modal('show')
    },
    confirmarBorrarPago() {
      const condicion = this.deleteCondicionEconomicaTemp
      if (condicion.estado == 'new') {
        this.condicioneseconomicas = this.condicioneseconomicas.filter(function(
          item
        ) {
          return item.idinterna !== condicion.idinterna
        })
      } else {
        this.condicioneseconomicas = this.condicioneseconomicas.map(item => {
          if (condicion.idinterna == item.idinterna) {
            item.estado = 'delete'
          }
          return item
        })
        this.conditionstosend.map(item => {
          if (condicion.idinterna == item.idinterna) {
            item.estado = 'delete'
          }
          return item
      ...
      this.condicioneseconomicas = this.condicioneseconomicas.filter(function(
        ...
        return item.estado != 'delete'
      })
      this.deleteCondicionEconomicaTemp = {}
      cash('#delete-modal-condicion-preview').modal('hide')
    },
    ...
    deleteService(service) {
      this.deleteServiceTemp = service
      this.confirmarEliminar()
    },
    confirmDelete() {
      const service = this.deleteServiceTemp
      if (service.estado == 'new') {
        ...
        this.servicesListProject = this.servicesListProject.map(item => {
    ...
    updateProject() {
      ...
      for (const key in this.servicesListProject) {
        ...
        if (element.listacostos.length > 0) {
          for (const jey in element.listacostos) {
            const elementj = element.listacostos[jey]
            detalle.push({
              estado: elementj.estado,
              id: elementj.id,
              tipo_costo_servicio_id: elementj.tipo_costo_servicio_id,
        ...
        serviciosTemp.push({
          ...
          otro_servicio: element.otroservicio ? element.otroservicio : '',
          ...
          otro_medio_pago: element.payment.otromediopago
            ? element.payment.otromediopago
            : '',
          pago_a_realizar: element.payment.responsablepago,
          detalle: detalle
        })
      }
      const condicionesTemp = this.conditionstosend.concat(
      ...
      const data = {
        ...
        condiciones_economicas: condicionesTemp,
        consumo_pago_estimado: [
          {
            id: 1,
            nombre: 'consumo_lubricante_dia',
            valor: this.consumocombustibledia
          },
          {
            id: 2,
            nombre: 'consumo_combustible_dia',
            valor: this.consumolubricantedia
          },
          {
            id: 3,
            nombre: 'consumo_refrigerante_dia',
            valor: this.consumorefrigerantedia
          },
          {
            id: 4,
            nombre: 'consumo_grasa',
            valor: this.consumograsa
          },
          {
            id: 5,
            nombre: 'consumo_llantas_dia',
            valor: this.consumollantasdia
          },
          {
            id: 6,
            nombre: 'pago_peajes',
            valor: this.pagopeajes
          },
          {
            id: 7,
            nombre: 'pago_conductor_dia',
            valor: this.pagoconductordia
          },
          {
            id: 8,
            nombre: 'pago_hidratacion_dia',
            valor: this.pagohidrataciondia
          },
          {
            id: 8,
            nombre: 'pago_hidratacion_dia',
          ...
          {
            id: 9,
            nombre: 'pago_parqueadero_dia',
            valor: this.pagoparqueaderodia
          },
          {
            id: 10,
            nombre: 'pago_soat_dia',
            valor: this.pagosoatdia
          },
          {
            id: 11,
            nombre: 'pago_tecnomecanica_dia',
            valor: this.pagotecnomecanicadia
          },
          {
            id: 12,
            nombre: 'pago_seguro_dia',
            valor: this.pagosegurodia
          },
          {
            id: 13,
            nombre: 'pago_leasing_dia',
            valor: this.pagoleasingdia
          },
          {
            id: 14,
            nombre: 'pago_lavado_dia',
            valor: this.pagolavadodia
          },
          {
            id: 15,
            nombre: 'pago_mantenimiento_dia',
            valor: this.pagomantenimientodia
          },
          {
            id: 16,
            nombre: 'pago_admin_dia',
            valor: this.pagoadmindia
          },
          {
            id: 17,
            nombre: 'salario_conductor_dia',
            valor: this.salarioconductordia
          },
          {
            id: 18,
            nombre: 'salario_gerencia_nacional_operaciones_dia',
            valor: this.salariogerencianalopedia
          },
          {
            id: 19,
            nombre: 'salario_gerencia_regional_operaciones_dia',
            valor: this.salariogerenciaregopedia
          },
          {
            id: 20,
            nombre: 'salario_gerencia_recursos_humanos_dia',
            valor: this.salariogerenciarrhhdia
          },
          {
            id: 21,
            nombre: 'salario_asistente_recursos_humanos_dia',
            valor: this.salarioasisrrhhdia
          },
          {
            id: 22,
            nombre: 'salario_gerencia_administracion_dia',
            valor: this.salariogerencaadmindia
          },
          {
            id: 23,
            nombre: 'salarios_upervisor_asignado_dia',
            valor: this.salariosupervidorasigdia
          },
          {
            id: 24,
            nombre: 'pago_arriendo_oficina_dia',
            valor: this.pagoarriendoofidia
          },
          {
            id: 25,
            nombre: 'pagos_servicios_oficina_dia',
            valor: this.pagoserviciosofidia
          },
          {
            id: 26,
            nombre: 'pago_alojamiento_dia',
            valor: this.pagoalojamientodia
          },
          {
            id: 27,
            nombre: 'pago_alimentacion_dia',
            valor: this.pagoalimentaciondia
          },
          {
            id: 28,
            nombre: 'pago_alquiler_camionetas_dia',
            valor: this.pagoalquilercamionetasdia
          },
          {
            id: 29,
            nombre: 'pago_tiquetes_aereos_dia',
            valor: this.pagotiquetesaereosdia
          },
          {
            id: 30,
            nombre: 'pago_transporte_terrestre_dia',
            valor: this.pagotransporteterrestredia
          },
          {
            id: 31,
            nombre: 'pago_gasolina_camionetas_dia',
            valor: this.pagogasolinacamionetasdia
          },
          {
            id: 32,
            nombre: 'pago_papeleria_dia',
            valor: this.pagopapeleriadia
          },
          {
            id: 33,
            nombre: 'pago_insumos_oficina_dia',
            valor: this.pagoinsumosofidia
          },
          {
            id: 34,
            nombre: 'pago_gastos_oficina',
            valor: this.pagogatosofi
          }
        ],
        costoServicio: serviciosTemp,
        tipo_vias: [
          {
            id:
              this.tipoviapavimentadaOLD == true &&
              this.tipoviapavimentada == false
                ? '1'
                : '',
            estado:
              this.tipoviapavimentadaOLD == true &&
              this.tipoviapavimentada == false
                ? 'delete'
                : this.tipoviapavimentadaOLD != this.tipoviapavimentada &&
                  'new',
            tipo_via_id:
              this.tipoviapavimentadaOLD == false &&
              this.tipoviapavimentada == true
                ? '1'
                : ''
          },
          {
            id:
              this.tipoviadestapadaOLD == true && this.tipoviadestapada == false
                ? '2'
                : '',
            estado:
              this.tipoviadestapadaOLD == true && this.tipoviadestapada == false
                ? 'delete'
                : this.tipoviadestapadaOLD != this.tipoviadestapada && 'new',
            tipo_via_id:
              this.tipoviadestapadaOLD == false && this.tipoviadestapada == true
                ? '2'
                : ''
          },
          {
            id:
              this.tipoviatrochaOLD == true && this.tipoviatrocha == false
                ? '3'
                : '',
            estado:
              this.tipoviatrochaOLD == true && this.tipoviatrocha == false
                ? 'delete'
                : this.tipoviatrochaOLD != this.tipoviatrocha && 'new',
            tipo_via_id:
              this.tipoviatrochaOLD == false && this.tipoviatrocha == true
                ? '3'
                : ''
          },
          {
            id:
              this.tipoviaotroOLD == true && this.tipoviaotro == false
                ? '4'
                : '',
            estado:
              this.tipoviaotroOLD == true && this.tipoviaotro == false
                ? 'delete'
                : this.tipoviaotroOLD != this.tipoviaotro && 'new',
            tipo_via_id:
              this.tipoviaotroOLD == false && this.tipoviaotro == true
                ? '4'
                : '',
            otros: this.otrotipodevia
          }
        ],
        rellenos: [
          {
            id:
              this.tipooperacionexcavacionOLD == true &&
              this.tipooperacionexcavacion == false
                ? '2'
                : '',
            estado:
              this.tipooperacionexcavacionOLD == true &&
              this.tipooperacionexcavacion == false
                ? 'delete'
                : this.tipooperacionexcavacionOLD !=
                    this.tipooperacionexcavacion && 'new',
            tipo_material_id:
              this.tipooperacionexcavacionOLD == false &&
              this.tipooperacionexcavacion == true
                ? '2'
                : ''
          },
          {
            id:
              this.tipooperacionrellenoOLD == true &&
              this.tipooperacionrelleno == false
                ? '1'
                : '',
            estado:
              this.tipooperacionrellenoOLD == true &&
              this.tipooperacionrelleno == false
                ? 'delete'
                : this.tipooperacionrellenoOLD != this.tipooperacionrelleno &&
                  'new',
            tipo_material_id:
              this.tipooperacionrellenoOLD == false &&
              this.tipooperacionrelleno == true
                ? '1'
                : ''
          },
          {
            id:
              this.tipooperacioninternoOLD == true &&
              this.tipooperacioninterno == false
                ? '3'
                : '',
            estado:
              this.tipooperacioninternoOLD == true &&
              this.tipooperacioninterno == false
                ? 'delete'
                : this.tipooperacioninternoOLD != this.tipooperacioninterno &&
                  'new',
            tipo_material_id:
              this.tipooperacioninternoOLD == false &&
              this.tipooperacioninterno == true
                ? '3'
                : ''
          },
          {
            id:
              this.tipooperacionotroOLD == true &&
              this.tipooperacionotro == false
                ? '4'
                : '',
            estado:
              this.tipooperacionotroOLD == true &&
              this.tipooperacionotro == false
                ? 'delete'
                : this.tipooperacionotroOLD != this.tipooperacionotro && 'new',
            tipo_material_id:
              this.tipooperacionotroOLD == false &&
              this.tipooperacionotro == true
                ? '4'
                : '',
            otros: this.otrotipooperacion
          }
      ...
      console.log(data)
      const fini1 = new Date(fechaIni)
      const ffin2 = new Date(fechaFin)
      if (ffin2.getTime() > fini1.getTime()) {
        this.errorFechas = false
      } else {
        this.errorFechas = true
      }
    ...
    getDataConditions(values) {
      if (values.formapago && values.paymentmethod && values.responsablepago) {
        if (values.nombre_condicion_economica_id == '4') {
            ...
            if (!condicion) {
              this.otroscondicioneseconomicas.push({
                ...
                nombre_condicion_economica_id:
                  values.nombre_condicion_economica_id,
          ...
          this.conditionstosend.push({
            id: values.id,
            estado: values.estado,
            nombre_condicion_economica_id: values.nombre_condicion_economica_id,
            forma_pago: values.formapago,
            medio_pago: values.paymentmethod,
            pago_a_realizar: values.responsablepago,
            otro_medio_pago: values.otromediopago,
          ...
          !this.conditionstosend.find(
            item =>
              item.nombre_condicion_economica_id ==
              values.nombre_condicion_economica_id
          )
        ) {
          this.conditionstosend.push({
    ...
    async GetProyecto() {
      ...
      this.loading = true
      await $s.getProyectoById(
        this.PARAMETROS.id,
        result => {
          const data = result.proyecto
          this.setUpData(data)
          this.loading = false
        },
        error => {
    ...
    async setUpData(data) {
      ...
      this.fechaInicio = data.fecha_inicio.replaceAll('-', '/')
      this.fechaFin = data.fecha_fin.replaceAll('-', '/')
      this.antiguedadvehiculos = data.antiguedad_vehiculo
      this.ubicacionInicial = data.ubicacion_inicial
      this.ubicacionFinal = data.ubicacion_final
      this.departamentoInicial = data.departamento_inicio_id
      this.departamentoFinal = data.departamento_final_id
      this.municipioInicio = data.municipio_inicio_id
      this.municipioFinal = data.municipio_final_id
      this.horasLaboral = data.horas_laboral_dia
      this.temperatura = data.temperatura
      this.dobletroque = await data.propietario_dobletroque
      this.duracionproyecto = data.duracion_proyecto

      this.valorcontrato = data.valor_contrato
      this.anticipodelcontrato = data.valor_anticipo_contrato
      this.antiguedadvehiculos = data.antiguedad_vehiculo
      this.otracaracteristicavehiculo =
        data.otros_requerimientos && data.otros_requerimientos
      const tiposvias = data.tipo_vias
      if (tiposvias.length > 0) {
        for (const key in tiposvias) {
          const element = tiposvias[key]
          switch (element.id) {
            case 1:
              this.tipoviapavimentada = true
              this.tipoviapavimentadaOLD = true
              break
            case 2:
              this.tipoviadestapada = true
              this.tipoviadestapadaOLD = true
              break
            case 3:
              this.tipoviatrocha = true
              this.tipoviatrochaOLD = true
              break
            case 4:
              this.tipoviaotro = true
              this.tipoviaotroOLD = true
              this.otrotipodevia = element.otros
          }
        }
      }
      const tipooperacion = data.rellenos
      if (tipooperacion.length > 0) {
        for (const key in tipooperacion) {
          const element = tipooperacion[key]
          switch (element.id) {
            case 1:
              this.tipooperacionrelleno = true
              this.tipooperacionrellenoOLD = true
            ...
            case 4:
              this.tipooperacionotro = true
              this.tipooperacionotroOLD = true
              this.otrotipooperacion = element.otros
          }
        }
      }
      const servicios = data.costo_servicio
      if (servicios.length > 0) {
        for (const key in servicios) {
          const servicio = servicios[key]
          const detalle = servicio.detalle
          const LISTACOSTOS = []
          for (const j in detalle) {
            const costo = detalle[j]
            LISTACOSTOS.push({
              estado: 'update',
              id: costo.id,
              tipo_costo_servicio_id: costo.tipo_costo_servicio_id,
              name: costo.nombre,
              nombre: costo.nombre,
              valor: costo.valor
            })
          }
          this.servicesListProject.push({
            estado: 'update',
            id: servicio.id,
            ...
            payment: {
              responsablepago: servicio.pago_a_realizar,
              formapago: servicio.forma_pago,
              paymentmethod: servicio.medio_pago
            },
            listacostos: LISTACOSTOS
          })
        }
      }
      this.servicesListProjectOLD = this.servicesListProject
      const recorridos = data.recorrido
      const listaRecorridos = []
      if (recorridos.length > 0) {
        for (const key in recorridos) {
          const recorrido = recorridos[key]
          listaRecorridos.push({
            estado: 'update',
            id: recorrido.id,
            recorrido_inicio_id: recorrido.recorrido_inicio_id,
            nombre_incio: recorrido.recorrido_inicio,
            recorrido_final_id: recorrido.recorrido_final_id,
            nombre_final: recorrido.recorrido_final,
            accion_id: recorrido.accion_id,
            nombre_accion: recorrido.accion
          })
        }
        this.recorridos = listaRecorridos
      }
      const condicioneseconomicas = data.condicones_economicas
      if (condicioneseconomicas.length > 0) {
        for (const key in condicioneseconomicas) {
          const element = condicioneseconomicas[key]
          this.condicioneseconomicas.push({
            estado: 'update',
            name: element.nombre_condicion_economica,
            nombre_condicion_economica_id:
              element.nombre_condicion_economica_id,
            id: element.id,
            idinterna: element.id,
            payment: {
              responsablepago: element.pago_a_realizar,
              formapago: element.forma_pago,
              paymentmethod: element.medio_pago,
              otromediopago: element.otromediopago && element.otromediopago
            }
          })
        }
      }
      this.condicioneseconomicasOLD = this.condicioneseconomicas
      const cosumogastoestimado = data.consumo_gasto_estimado
      for (const key in cosumogastoestimado) {
        const element = cosumogastoestimado[key]
        switch (element.nombre) {
          case 'consumo_combustible_dia':
            this.consumocombustibledia = element.valor
            break
          case 'consumo_lubricante_dia':
            this.consumolubricantedia = element.valor
            break
          case 'consumo_refrigerante_dia':
            this.consumorefrigerantedia = element.valor
            break
          case 'consumo_grasa':
            this.consumograsa = element.valor
            break
          case 'consumo_llantas_dia':
            this.consumollantasdia = element.valor
            break
          case 'pago_peajes':
            this.pagopeajes = element.valor
            break
          case 'pago_conductor_dia':
            this.pagoconductordia = element.valor
            break
          case 'pago_hidratacion_dia':
            this.pagohidrataciondia = element.valor
            break
          case 'pago_parqueadero_dia':
            this.pagoparqueaderodia = element.valor
            break
          case 'pago_soat_dia':
            this.pagosoatdia = element.valor
            break
          case 'pago_tecnomecanica_dia':
            this.pagotecnomecanicadia = element.valor
            break
          case 'pago_seguro_dia':
            this.pagosegurodia = element.valor
            break
          case 'pago_lavado_dia':
            this.pagolavadodia = element.valor
            break
          case 'pago_mantenimiento_dia':
            this.pagomantenimientodia = element.valor
            break
          case 'pago_admin_dia':
            this.pagoadmindia = element.valor
            break
          case 'salario_conductor_dia':
            this.salarioconductordia = element.valor
            break
          case 'salario_gerencia_nacional_operaciones_dia':
            this.salariogerencianalopedia = element.valor
            break
          case 'salario_gerencia_regional_operaciones_dia':
            this.salariogerenciaregopedia = element.valor
            break
          case 'salario_gerencia_recursos_humanos_dia':
            this.salariogerenciarrhhdia = element.valor
            break
          case 'salario_asistente_recursos_humanos_dia':
            this.salarioasisrrhhdia = element.valor
            break
          case 'salario_gerencia_administracion_dia':
            this.salariogerencaadmindia = element.valor
            break
          case 'salarios_upervisor_asignado_dia':
            this.salariosupervidorasigdia = element.valor
            break
          case 'pago_arriendo_oficina_dia':
            this.pagoarriendoofidia = element.valor
            break
          case 'pagos_servicios_oficina_dia':
            this.pagoserviciosofidia = element.valor
            break
          case 'pago_alojamiento_dia':
            this.pagoalojamientodia = element.valor
            break
          case 'pago_alimentacion_dia':
            this.pagoalimentaciondia = element.valor
            break
          case 'pago_alquiler_camionetas_dia':
            this.pagoalquilercamionetasdia = element.valor
            break
          case 'pago_tiquetes_aereos_dia':
            this.pagotiquetesaereosdia = element.valor
            break
          case 'pago_transporte_terrestre_dia':
            this.pagotransporteterrestredia = element.valor
            break
          case 'pago_gasolina_camionetas_dia':
            this.pagogasolinacamionetasdia = element.valor
            break
          case 'pago_papeleria_dia':
            this.pagopapeleriadia = element.valor
            break
          case 'pago_insumos_oficina_dia':
            this.pagoinsumosofidia = element.valor
            break
          case 'pago_gastos_oficina':
            this.pagogatosofi = element.valor
            break
        }
      }
      this.cantidadasignadopropio = data.cantidad_vehiculo_propio
      this.cantidadasignadoalquilado = data.cantidad_vehiculo_alquilado
      this.valorm3propio = data.valor_metrocubico_propio
      this.valorm3alquilado = data.valor_metrocubico_alquilado
      this.file = {
        id: data.contratos[0].id,
        nombre: data.contratos[0].nombre,
        extension: data.contratos[0]['extension archivo'],
        ruta: data.contratos[0].ruta
      }
    }
  },
  watch: {
    ...
    valorcontrato: {
      handler(newVal) {
        if (parseInt(newVal) <= parseInt(this.anticipodelcontrato)) {
  ...
  computed: {
    ...
    totalcantidades() {
      console.log(this.cantidadasignadoalquilado)
      console.log(this.cantidadasignadopropio)
      return parseInt(this.cantidadasignadopropio ? this.cantidadasignadopropio : 0) + parseInt(this.cantidadasignadoalquilado ? this.cantidadasignadoalquilado : 0)
    },
    totaldatosoperacion() {
      const total =
